    ...
    <div className="flex flex-col w-full h-full pb-10">
      ...
      <div className="px-[1rem] md:px-[2.5rem] py-[1.5rem] border-b border-white/[0.07] grid grid-cols-2 md:grid-cols-4 gap-y-6 md:gap-x-0 md:gap-y-0 divide-x-0 md:divide-x divide-white/[0.07]">
        <div className="flex flex-col gap-[0.3rem] pr-[1rem] md:pr-0 md:mr-[2rem] border-r-0 md:border-r border-white/[0.07]">
          ...
          <div className="font-display font-[900] text-[2.6rem] tracking-[-0.04em] text-white leading-none">
        ...
        <div className="flex flex-col gap-[0.3rem] pl-[1rem] md:pl-0 pr-[1rem] md:pr-0 md:ml-[2rem] md:mr-[2rem] border-r-0 md:border-r border-white/[0.07]">
          ...
          <div className="font-mono text-[0.68rem] text-[#00bfdb]">
        ...
        <div className="flex flex-col gap-[0.3rem] pr-[1rem] md:pl-0 md:pr-0 md:ml-[2rem] md:mr-[2rem] border-r-0 md:border-r border-white/[0.07]">
          ...
          <div className="font-mono text-[0.68rem] flex items-center gap-[0.4rem]">
            <div
              className="w-[5px] h-[5px] rounded-full bg-[#00bfdb] opacity-80"
              style={{ animation: "pulse-opacity 1.8s ease-in-out infinite" }}
            />
        ...
        <div className="flex flex-col gap-[0.3rem] pl-[1rem] md:pl-0 md:ml-[2rem] border-r-0 border-white/[0.07]">
          ...
          <div className="font-mono text-[0.68rem] text-white/40">
            / 100 index
          </div>
        </div>
      </div>
      ...
      {/* Zone 4 — Charts Row */}
      <div
        className="px-[1rem] md:px-[2.5rem] py-[2rem] border-b border-white/[0.07] grid grid-cols-1 md:grid-cols-2 gap-[2.5rem] lg:gap-[4rem]"
      ...
      >
        ...
        >
          <div className="font-mono text-[0.65rem] uppercase text-white/40 mb-[1.2rem]">
            COGNITIVE LOAD DISTRIBUTION
          </div>
          <div className="relative w-full flex-1 flex items-center justify-center">
            <canvas
              ref={doughnutRef}
              className="w-full h-full bg-transparent max-h-[300px]"
            ></canvas>
            <div
              className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none"
              style={{ transform: "translateX(-22%)" }}
            >
              <div className="font-display font-[800] text-[1.8rem] leading-none text-white">
                {analytics.cognitiveLoad.distribution.high +
                  analytics.cognitiveLoad.distribution["very-high"]}
              </div>
              <div className="font-mono text-[0.6rem] text-white/40 uppercase mt-1">
                High Vol
              </div>
            </div>
          </div>
      ...
      <div className="px-[1rem] md:px-[2.5rem] py-[2.5rem]">
        ...
        <div className="w-full text-left">
          ...
          {platformData.map((row, idx) => (
            <div
              key={idx}
              className="grid grid-cols-1 md:grid-cols-5 items-center py-[1rem] border-b border-white/[0.04] group hover:bg-white/[0.01] transition-colors gap-2 md:gap-0"
            >
              <div className="col-span-2 font-mono text-[0.8rem] text-white/80 group-hover:text-[#00bfdb] transition-colors">
                {row.platform}
              </div>
              <div className="font-mono text-[0.8rem] text-white/60">
                {row.sessions} total
              </div>
              <div className="font-mono text-[0.8rem] text-white/60">
                {Math.round(row.avgDuration / 60)}m{" "}
                {Math.round(row.avgDuration % 60)}s
              </div>
...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ onLogout }) => {
  ...
  const [activeView, setActiveView] = useState<
    "overview" | "participants" | "sessions" | "behavioral"
  >("overview");
  ...
  if (loading) {
    return (
      <div className="min-h-[100dvh] flex items-center justify-center bg-[#090a0c] text-white">
  ...
  if (error) {
    return (
      <div className="min-h-[100dvh] flex items-center justify-center bg-[#090a0c] text-white px-6">
        <div className="max-w-xl w-full border border-red-400/30 bg-red-900/10 rounded-md p-4">
          <div className="font-display text-base text-red-200 mb-1">
            Failed to load admin data
          </div>
          <div className="font-mono text-sm text-red-100/80">{error}</div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-[100dvh] flex flex-col bg-[#090a0c] text-white text-left selection:bg-[#00bfdb] selection:text-[#090a0c]">
      ...
      <div className="sticky top-0 z-10 flex justify-between items-center px-[2.5rem] py-[1rem] border-b border-white/[0.07] bg-[#090a0c]/95 backdrop-blur-[14px]">
        <div>
          <div className="font-mono text-[0.65rem] uppercase text-white/40 mb-[0.2rem]">
            COGNITIVE LOAD LAB
          </div>
          <h1 className="font-display font-[800] text-[1.1rem] text-white leading-none">
            Admin Dashboard
          </h1>
          <div className="font-mono text-[0.68rem] text-white/40 mt-[0.3rem]">
      ...
      <div className="flex-1 flex flex-col min-h-0 relative">
        {activeView === "overview" && analytics && (
          <AdminOverviewTab analytics={analytics} />
        )}
        ...
        <div
          className={
            activeView !== "overview"
              ? "max-w-7xl mx-auto px-6 py-8 w-full text-left"
              : "hidden"
          }
        >
          {activeView === "participants" && (
            <section className="border border-white/[0.08] bg-[#0b0d10] rounded-[10px] overflow-hidden">
              <div className="px-6 py-5 border-b border-white/[0.07] flex items-end justify-between gap-4">
                <div>
                  <div className="font-mono text-[0.65rem] uppercase tracking-wider text-white/35">
                    Participant Registry
                  </div>
                  <h3 className="font-display font-[800] text-[1.05rem] text-white mt-1">
                    All Participants
                  </h3>
                </div>
                <div className="font-mono text-[0.72rem] text-white/45">
                  Total:{" "}
                  <span className="text-[#00bfdb]">{participants.length}</span>
                </div>
              </div>
                ...
                <table className="min-w-full">
                  <thead>
                    <tr className="border-b border-white/[0.07] bg-white/[0.01]">
                      <th className="px-6 py-3 text-left font-mono text-[0.65rem] uppercase tracking-wider text-white/35">
                        Name
                      </th>
                      <th className="px-6 py-3 text-left font-mono text-[0.65rem] uppercase tracking-wider text-white/35">
                        Email
                  ...
                  <tbody>
                    {participants.map((participant) => (
                      ...
                      >
                        <td className="px-6 py-4 whitespace-nowrap font-display text-[0.95rem] text-white/90">
                          {participant.name}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap font-mono text-[0.75rem] text-white/55">
                          {participant.email}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span
                            className={`px-2 py-1 rounded-full font-mono text-[0.65rem] uppercase tracking-wide border ${
                              participant.role === "admin"
                                ? "text-[#00bfdb] border-[#00bfdb]/40 bg-[#00bfdb]/10"
                                : "text-white/65 border-white/20 bg-white/[0.03]"
                            }`}
                          >
                            {participant.role}
                          </span>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap font-mono text-[0.75rem] text-white/65">
                          {participant.sessions?.[0]?.count || 0} total
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap font-mono text-[0.75rem] text-white/45">
                          {new Date(
          ...
          {activeView === "sessions" && (
            <section className="border border-white/[0.08] bg-[#0b0d10] rounded-[10px] overflow-hidden">
              <div className="px-6 py-5 border-b border-white/[0.07] flex items-end justify-between gap-4">
                ...
                <div className="font-mono text-[0.72rem] text-white/45">
                  Total:{" "}
                  <span className="text-[#00bfdb]">{sessions.length}</span>
                </div>
              </div>
                  ...
                  <tbody>
                    {sessions.map((session) => (
                      <tr
                        ...
                        className="border-b border-white/[0.05] hover:bg-white/[0.02] transition-colors"
                      >
                        <td className="px-6 py-4 whitespace-nowrap font-display text-[0.9rem] text-white/90">
                          {session.participants?.name || "Unknown"}
                        </td>
                        <td className="max-w-xs truncate px-6 py-4 font-mono text-[0.75rem] text-white/55">
                          {session.topic}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap font-mono text-[0.72rem] uppercase text-[#00bfdb]">
                          {session.platform}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span
                            className={`px-2 py-1 rounded-full font-mono text-[0.65rem] uppercase tracking-wide border ${
                              session.current_phase === "completed"
                                ...
                                : session.current_phase === "research"
                                  ? "text-white/75 border-white/20 bg-white/[0.03]"
                        ...
                        <td className="px-6 py-4 whitespace-nowrap font-mono text-[0.75rem] text-white/65">
                          {session.cognitive_load_metrics?.[0] ? (
                            <span
                              className={`px-2 py-1 rounded-full font-mono text-[0.65rem] border ${
                                session.cognitive_load_metrics[0].category ===
                                "low"
                                  ? "text-[#00bfdb] border-[#00bfdb]/40 bg-[#00bfdb]/10"
                                  : session.cognitive_load_metrics[0]
                                        .category === "moderate"
                            ...
                            >
                              {session.cognitive_load_metrics[0].overall_score}{" "}
                              ({session.cognitive_load_metrics[0].category})
                            </span>
                          ) : (
          ...
          {activeView === "behavioral" && (
            <section className="space-y-6">
              {/* Filters */}
              <div className="border border-white/[0.08] bg-[#0b0d10] rounded-[10px] p-6">
                <div className="font-mono text-[0.65rem] uppercase tracking-wider text-white/35 mb-1">
                  Query Controls
                </div>
                <h3 className="font-display text-[1.05rem] font-[800] text-white mb-5">
                  Behavioral Filters
                </h3>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {/* Platform Filter */}
                  <div>
                    <label className="mb-2 block font-mono text-[0.68rem] uppercase tracking-wide text-white/45">
                      Platform
                    </label>
                    <select
                      ...
                      className="w-full bg-[#0f1114] border border-white/[0.15] rounded-[7px] px-3 py-2 text-white/80 font-mono text-[0.78rem] focus:outline-none focus:border-[#00bfdb]"
                    >
              ...
              {behavioralTimeline.length > 0 && (
                <div className="border border-white/[0.08] bg-[#0b0d10] rounded-[10px] p-6">
                  <h3 className="mb-4 font-display text-[1.05rem] font-[800] text-white">
              ...
              {behavioralTimeline.length > 0 && (
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                  {[
                    ...
                    const total = behavioralTimeline.reduce(
                      (sum, day: any) => sum + (day[category.key] || 0),
                      0,
                    );
                    const percentage =
                      (behavioralTimeline.reduce(
                        (sum, day: any) => sum + (day[category.key] || 0),
                        0,
                      ) /
                        behavioralTimeline.reduce(
                          (sum, day: any) => sum + (day.total || 0),
                          0,
                        )) *
                      100;
                    return (
                      <div
                        key={category.key}
                        className="border border-white/[0.08] bg-[#0b0d10] rounded-[10px] p-6"
                      >
                        <div className="flex items-center justify-between">
                          <div>
                            <p className="font-mono text-[0.7rem] uppercase tracking-wide text-white/45">
                              {category.label}
                            </p>
                            <p className="mt-2 font-display text-2xl font-[900] text-white">
                              {total}
                            </p>
                ...
                <div className="border border-white/[0.08] bg-[#0b0d10] rounded-[10px] p-6">
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full">
                      <thead className="border-b border-white/[0.07] bg-white/[0.01]">
                        <tr>
                          <th className="px-6 py-3 text-left font-mono text-[0.65rem] uppercase tracking-wider text-white/35">
                            Participant
                          </th>
                      ...
                      <tbody>
                        {behavioralPredictions.slice(0, 20).map((pred, idx) => (
                          ...
                          >
                            ...
                            <td className="px-6 py-4 whitespace-nowrap font-mono text-[0.72rem] uppercase text-[#00bfdb]">
                              <span>{pred.sessions?.platform || "N/A"}</span>
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap">
                              <span
                                className={`px-2 py-1 rounded-full font-mono text-[0.65rem] border ${
                                  pred.predicted_load_category === "low"
                                    ? "text-[#00bfdb] border-[#00bfdb]/40 bg-[#00bfdb]/10"
                                    : pred.predicted_load_category ===
                                        "moderate"
                                      ? "text-[#f59e0b] border-[#f59e0b]/40 bg-[#f59e0b]/10"
                                      : pred.predicted_load_category === "high"
                                        ? "text-[#fb7185] border-[#fb7185]/40 bg-[#fb7185]/10"
                                        : "text-[#ef4444] border-[#ef4444]/40 bg-[#ef4444]/10"
                                }`}
                              >
              ...
              {behavioralPredictions.length === 0 && (
                <div className="border border-white/[0.08] bg-[#0b0d10] rounded-[10px] p-12 text-center">
                  <Activity className="mx-auto mb-4 h-12 w-12 text-white/35" />
                  <p className="font-mono text-[0.8rem] text-white/45">
                    No behavioral predictions available for the selected filters
                  </p>
                </div>
              )}
            </section>
      ...
      {/* Zone 6 — Footer Strip */}
      <footer className="border-t border-white/[0.07] px-[2.5rem] py-[0.85rem] flex flex-col md:flex-row gap-2 justify-between items-center mt-auto bg-[#090a0c] z-10 shrink-0">
        <div className="font-mono text-[0.68rem] text-white/40">
          Cognitive Load Lab · Administrator View · IRB Approved
        </div>
        <div className="font-mono text-[0.68rem] text-white/40">
          n={analytics?.overview.totalParticipants || 0} participants · {"2"}{" "}
          active arms
        </div>
      </footer>
    </div>
